...
import { Music2, Heart, ListMusic, Plus, FolderOpen, FileAudio, Loader2, ChevronRight, Settings as SettingsIcon } from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      <div className="p-4 pb-3">
        {importing ? (
          <div className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-[var(--color-glass-mid)] border border-[var(--color-border-mid)] text-white/70 text-sm font-medium">
          ...
          <DropdownMenu.Root>
            <DropdownMenu.Trigger asChild>
              <button
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-[var(--color-glass-mid)] border border-[var(--color-border-mid)] text-white/70 text-sm font-medium hover:bg-[var(--color-glass-strong)] hover:text-white/90 hover:border-[var(--color-border-mid)] active:scale-[0.98] transition-all duration-150"
              >
                <FolderOpen size={13} />
                <span>Add Music</span>
              </button>
            ...
            <DropdownMenu.Portal>
              <DropdownMenu.Content
                className="z-[200] min-w-52 p-1 rounded-xl border border-[var(--color-border-mid)] text-sm"
                style={{ background: 'var(--color-chrome)', backdropFilter: 'blur(20px)' }}
                sideOffset={6} align="start">
                <DropdownMenu.Item onClick={importFolder}
                ...
                <DropdownMenu.Item onClick={importFiles}
                  className="flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer text-white/60 hover:text-white/90 hover:bg-white/5 outline-none transition-colors">
                  <FileAudio size={13} />
                  Add Files...
                </DropdownMenu.Item>
