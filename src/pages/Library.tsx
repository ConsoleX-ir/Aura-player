...
export function Library() {
  const { library, activeView, favorites } = usePlayerStore()
      ...
      <div className="flex-1 px-7 pb-4 overflow-y-auto">
        ...
        {library.length === 0 && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            className="flex flex-col items-center justify-center h-60 gap-5">
            <div className="relative">
        ...
        {viewMode === 'list' && songs.length > 0 && (
          <div>
            ...
            {songs.map((song, i) => <SongRow key={song.id} song={song} index={i} queue={songs} />)}
          </div>
        )}
